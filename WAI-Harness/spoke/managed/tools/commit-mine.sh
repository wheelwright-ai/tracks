...
#
#   - you pass `-- <paths>`                          -> ALWAYS stage only those
#       paths, regardless of isolation/liveness. Explicit scope is never
#       overridden by an "you own the tree" verdict.
#   - no paths + isolated worktree, OR sole live session -> you own the tree;
#       `git add -A` is safe; commit everything.
#   - no paths + shared tree + other live sessions    -> NEVER add -A; refuse
#       (exit 3) and require you to name your files.
#
...
if [ "${#PATHS[@]}" -gt 0 ]; then
  echo "commit-mine: explicit scope given — staging ONLY your ${#PATHS[@]} path(s) (isolated=$ISOLATED live=$LIVE)."
  git add -- "${PATHS[@]}"
elif [ "$ISOLATED" = 1 ] || [ "${LIVE:-1}" -le 1 ]; then
