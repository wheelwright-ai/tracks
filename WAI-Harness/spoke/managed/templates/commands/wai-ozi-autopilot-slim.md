...
`{harness_path}` = value of `.wheel.harness_path` in `WAI-Harness/spoke/WAI-State.json`.
...
If `routed_to: "BASHER"` → move to `WAI-Harness/spoke/lugs/incoming/` of basher spoke.
...
| File | Purpose |
|------|---------|
| `WAI-Harness/spoke/advisors/autopilot/scan_state.json` | Run stats, last run timestamp |
| `WAI-Harness/spoke/advisors/autopilot/activity-log.jsonl` | Every run, append-only |
