...
DIRECT_TEACH_NEW=0
DIRECT_TEACH_NAMES=''
for _dt_dir in "$PROJECT_DIR/WAI-Spoke/seed/ingest/manual" "$PROJECT_DIR/WAI-Spoke/seed/ingest/incoming"; do
  [[ -d "$_dt_dir" ]] || continue
...
# agent decides what to work on. Loud on absent/degraded: silence is how a
# dead system stays hidden. Ships together with tools/tastegraph_vector.py;
# without that tool this prints ABSENT rather than failing quietly.
AWARENESS_SECTION=""
...
if [[ -f "$_TGV_TOOL" ]]; then
  ...
  [[ -z "$AWARENESS_SECTION" ]] && AWARENESS_SECTION="AWARENESS VECTOR: DEGRADED — selector present but returned nothing. Orienting preferences are NOT in force."
else
