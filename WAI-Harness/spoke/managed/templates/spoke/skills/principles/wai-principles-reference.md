...
Lugs are JSONL records in `WAI-Harness/spoke/lugs/active/WAI-Lugs-active.jsonl` (active) or individual files in `WAI-Harness/spoke/lugs/{type}/` (archived). The active lugs file is the only storage the wakeup protocol reads at session start. When a session ends or is interrupted, lugs survive. Everything else is gone.
