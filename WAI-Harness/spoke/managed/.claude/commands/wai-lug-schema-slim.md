...
Optional fields any lug MAY carry — part of the lug certification chain. Overlay onto existing status dirs (`open`/`in_progress`/`completed`), no directory changes. Full detail + provably-additive evidence: `wai-lug-schema.md`.

| Field | Meaning |
|-------|---------|
| `cert_state` | `DRAFTED` \| `VALIDATED` \| `SCHEDULED` \| `IMPLEMENTED` \| `VERIFIED` \| `RECONCILED` — maps onto open/in_progress/completed |
| `cert_weight` | `trivial` \| `standard` \| `critical` |
| `signatures[]` | append-only `{gate, role, agent_id, model, verdict, timestamp, attests, evidence[], findings?}` |

**Bootstrap exception:** Ozi is manual gate owner until the Gate 3/4 sub-agents exist — treat these fields as informative/manually-maintained until then.
