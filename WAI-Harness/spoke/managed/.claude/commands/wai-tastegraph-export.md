...
**Args:**
- `--interface` — which graph to export. Defaults to `WAI-Harness/spoke/tastegraph.json` (personal-collaboration). Pass a context slug to load from `WAI-Harness/spoke/tastegraphs/<slug>.json`.
...
- No `--interface` arg → load `WAI-Harness/spoke/tastegraph.json`
- `--interface <slug>` → load `WAI-Harness/spoke/tastegraphs/<slug>.json`
