...
# Resolve this session's lane (idempotent LOOKUP, never a create). Yields the lane's own
# track + private runtime dir; falls through to the legacy routing below when absent.
#
# --no-create is LOAD-BEARING (impl-lane-registry-liveness-authority): this is a Stop
# hook — a pure HEARTBEAT — and a heartbeat must NEVER mint a lane. lane-resolve aliases
# lane-register with create=True, so this hook used to "lazy-create" a lane for a session
# that had already ended, RESURRECTING it after SessionEnd unregistered it (the ghost-lane
# bug: an 11.4h phantom peer holding the tree). Losing the lazy-create costs nothing here:
# the fallback below routes the turn by _session_state.track_path / newest session dir, so
# the turn is still captured — it just no longer authors a liveness claim on the way.
TRACK=""
LANE_DIR=""
_WG="$PROJECT_DIR/WAI-Harness/spoke/managed/tools/worktree_guard.py"
if [[ -n "$CC_SID" && -f "$_WG" ]]; then
  _STF_LANE_ERR=$(mktemp 2>/dev/null || echo /tmp/wai-stf-lane-err.$$)
  if ! _LANE=$(python3 "$_WG" lane-resolve --session "$CC_SID" --base "$BASE" --transcript "$TRANSCRIPT" --no-create 2>"$_STF_LANE_ERR"); then
    # Loud, not silent: a broken lane lookup means this turn may land in the wrong
    # session's track. Announce, then fall through to the legacy routing below.
    echo "[WAI] degraded: lane-resolve failed in stop-track-flush — $(head -c 300 "$_STF_LANE_ERR" 2>/dev/null | tr '\n' ' '). Falling back to legacy track routing." >&2
    _LANE=""
  fi
  rm -f "$_STF_LANE_ERR" 2>/dev/null
  if [[ -n "$_LANE" ]]; then
