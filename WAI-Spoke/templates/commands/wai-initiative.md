...
Read from `WAI-Spoke/initiatives/index.json` → `themes[]`.
...
For each initiative in `WAI-Spoke/initiatives/index.json` → `initiatives[]`:
...
| Data | Source |
|------|--------|
| Theme definitions + scores | `WAI-Spoke/initiatives/index.json` → `themes[]` |
| Initiative groups | `WAI-Spoke/initiatives/index.json` → `initiatives[]` |
