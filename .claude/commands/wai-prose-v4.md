...
**0. Intent check.** Read `{BASE}/runtime/session-intent.json` (authoritative), falling back to
an `Intent:` line in `<wai-session-init>` CONTEXT HEALTH.
- If found: extract `SESSION_INTENT` and `SESSION_INTENT_LABEL`. After the Step 2 banner, go to
  **Step 2b** (intent router).
  - Steps 3 and 4 are skipped **only for an intent that actually selected work** (`savepoint`,
    `implement`, `refinement`, `teachings`, `closeout`). `raw`, `explore`, and any value the
    router does not list select nothing — they fall through to Step 2c and then Steps 3+4.
...
**Whatever the intent, Step 2c still runs.** No intent value may suppress the savepoint claim;
only an actual claim (or an explicit operator decline) clears it.
...
**2a.3 — Session ARC read (MANDATORY, one call, impl-wakeup-reads-the-session-arc-v1).**

Everything above tells you WHERE things are. None of it tells you how the thinking GOT
there — the order a problem was understood in, what was tried, what that ruled out. Operator,
2026-08-07: *"the agent doing a read of the track file would be a big assistance to the
progression of a thought/action."*

```bash
python3 {TOOLS}/track_arc.py --base {BASE}
```

Print the output verbatim, before the work queue. It is bounded (12 turns, clipped lines) and
silent when there is no history, so it costs one call and cannot fail a wakeup.

**Read the header honestly, because it is telling you how much to trust the rest.** Measured
across 142 real turn rows: only 16% are model-authored and carry WHY; 66% are derived from the
transcript by the Layer-2 net and carry only WHAT. A line marked `(no reasoning recorded)` is a
turn whose rationale was never written down — treat it as **unrecorded, never as idle, and
never as a decision**. When the header says `thin:`, the previous session left you actions
without reasons, and you should re-derive rather than assume.

Do NOT read `track.jsonl` directly instead. Tracks run 24-95KB, two thirds of it transcript
echo; the arc is the fraction that carries the thread.

**Your own turns feed the next session's arc.** The per-turn buffer merge (`user_intent`,
`action`, `outcome`, `thinking`, `focus`, `phase`, plus `decisions`/`open`) is what makes a row
count as WHY. A turn you do not fill lands as `(no reasoning recorded)` for whoever wakes next.
...
Then, for each unadopted teaching listed in `{BASE}/runtime/teachings-discovery.json` (teachings render is deprecated at wakeup — this file is the discovery source now, see `new_teachings_raw`):
...
| `raw` | — | The operator declined the opening prompt (`wai-enter.sh` choice `r`, or `-r`/`--raw`/`--resume`). This selects NO work — it is not a decision to abandon a pending savepoint. **Fall through to Step 2c, then Steps 3+4 normally.** |
...
| *(any other value)* | — | An intent this table does not list is NOT a decision about the savepoint. **Fall through to Step 2c, then Steps 3+4** — never silently skip. |

After routing, **do not ask for vibe**. Proceed directly to work.

**2c. Savepoint intercept — CLAIM, do not merely display.**

Runs whenever `_savepoint` is pending and Step 2b did not already claim it. **The gate is
"has it been claimed", not "was an intent present"** — an intent that says nothing about the
savepoint (`raw`, `explore`, anything unlisted) must not suppress this step.

Measured failure this rule exists to prevent (session 140, 2026-08-06): intent was `raw`,
2b had no `raw` row, and 2c was gated on "SESSION_INTENT is absent" — so the savepoint was
displayed in the briefing, recommended in prose, and left at `pending`. Nothing owned it. The
operator had to ask whether it had been picked up, which is a question the state file should
answer on its own.

Run the tool — it reads state directly, so it cannot be fooled by a stale or absent brief:

```bash
python3 {TOOLS}/savepoint_claim.py --base {BASE} status --json
```

If `pending` is true, **claim it** (do not ask first — a pending savepoint is work the last
session explicitly handed forward, and the operator's standing directive is to take the
obvious action rather than spend a turn on permission):

```bash
python3 {TOOLS}/savepoint_claim.py --base {BASE} claim --session-id {session_id}
```

The claim is atomic and idempotent. It sets `_savepoint.status = "resumed"`, stamps
`resumed_at`/`resumed_by_session`, mirrors the status onto the canonical savepoint file, and
applies the focus lock (`_session_state.active_initiative_id` + `focus_directive`) when the
savepoint carries one.

Then report — in this order, magnitude first:
1. `⚑ Claimed: {savepoint_id} — {lug_id}`
2. `Initiative: {initiative_id}` and the focus directive, when present
3. `Next: {resume_note}`
4. The ordered queue, if the savepoint carries `ordered_queue`, with each item's `why`
5. `waiting_on_operator`, when present — these are the operator's calls, not yours

Verify the top queue lugs actually exist on disk before presenting them as the plan. A queue
naming lugs that were since moved or deleted is a dead end dressed as a plan.

**To decline instead:** the operator says so, and you clear `_savepoint = {}` and continue to
Step 3. Declining is one sentence from them; claiming costs them nothing. That asymmetry is
why claim is the default.

**Focus lock:** once claimed, stay on `initiative_id` for the session. Record anything found
outside that silo as a `notation` lug with `status: deferred` and
`deferred_from_initiative: {initiative_id}` — do not act on it.

**Assurance (end of wakeup, and again at closeout):**

```bash
python3 {TOOLS}/savepoint_claim.py --base {BASE} assert-claimed
```

Exit 1 means a savepoint is still pending — the ceremony dropped it. Fix it before reporting
the wakeup complete. This is the oracle that makes the failure detectable instead of silent.
...
- Finish the WAI Point briefing before pausing for approval on any side action.
- **Teachings are deprecated as a wakeup-rendered ceremony (feedback-hide-teachings-from-wakeup): never render a "Teachings" or "Pending Teachings" line, and never pause the briefing on teaching count.** The underlying adoption mechanism (Section 5B) and discovery data still exist for an explicit query path — this only silences the automatic wakeup surface.
- **Incoming lugs are first-class priority.** If non-zero at wakeup, address before entering work queue mode. Do not skip or defer to the next session.
...
- If stale-task decisions need approval, list them compactly under `Pending Items` inside the briefing rather than stopping early.
