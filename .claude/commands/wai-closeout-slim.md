...
```bash
WAI-Harness/spoke/managed/tools/closeout.sh --modified-by {model_id} --track-path {current_track_path}
```
...
try:
    current_session_id = json.load(open(resolve_guard_path('{BASE}', os.environ.get('CLAUDE_CODE_SESSION_ID')))).get('session_id','')
...
sys.path.insert(0, 'WAI-Harness/spoke/managed/tools')
from worktree_guard import resolve_guard_path
guard = json.load(open(resolve_guard_path('{BASE}', os.environ.get('CLAUDE_CODE_SESSION_ID'))))
...
existing = [f for f in glob.glob('{BASE}/initiatives/savepoints/**/*.json', recursive=True) if 'completed' not in Path(f).parts]
has_sp = any(
...
if next_rec and next_rec not in ('None', '', 'none') and not has_sp:
    ...
    init_dir = sp['initiative_id'] or 'initiative-unfiled-savepoints-v1'
    Path(f'{BASE}/initiatives/savepoints/{init_dir}').mkdir(parents=True, exist_ok=True)
    Path(f'{BASE}/initiatives/savepoints/{init_dir}/{sp_id}.json').write_text(json.dumps(sp, indent=2) + '\n')
...
```python
import json, datetime, subprocess, os, re, sys
from datetime import timezone
sys.path.insert(0, 'WAI-Harness/spoke/managed/tools')
from worktree_guard import resolve_guard_path
session_id = json.load(open(resolve_guard_path('{BASE}', os.environ.get('CLAUDE_CODE_SESSION_ID')))).get('session_id', 'unknown')
...
Emit a `kind=ceremony` bolt certifying closeout completed. Skip if `WAI-Harness/spoke/managed/tools/verify_engine.py` absent.
...
if [[ -f "WAI-Harness/spoke/managed/tools/verify_engine.py" ]]; then
    python3 WAI-Harness/spoke/managed/tools/verify_engine.py emit-ceremony \
