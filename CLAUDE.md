# CLAUDE.md (generated -- never hand-edit; regenerated on every real cut)

This file is a real, mechanical pointer into this spoke's own canon --
not a second, hand-maintained copy of policy prose. Run the compiler
(`wcl compile .`) for the authoritative, current state;
this file only summarizes what canon declared as of the last cut.
...
**Ambassador Brief:** see `AMBASSADOR_BRIEF.md` (generated) -- what this
spoke is, its toolbox, how the wheel works around it, where to ask for
centralized knowledge, how to report up, how to optimize performance,
and the preferred way to do a task where more than one tool exists.
...
- **conversation-track-ingestion** (on_demand): an operator or agent runs `node scripts/ingest-conversations.js` at an instance root, to mine conversation/track exports from any declared source for operator goals and preferences and for stranded ideas.
